...
__all__ = ['ep_resnet10', 'ep_resnet18', 'ep_resnet34', 'ep_resnet50']

class EP_ResNet(ResNet):
    def __init__(self, block, layers, exit_module=None, start_mark=0, selection=False, num_classes=100):
        super(EP_ResNet, self).__init__(block, layers, num_classes)
        self.start_mark = start_mark

        self.exit_module = nn.ModuleList(
            [exit_module(channels=64, b_stride=8, selection=selection, num_classes=num_classes),
             exit_module(channels=128, b_stride=4, selection=selection, num_classes=num_classes),
             exit_module(channels=256, b_stride=2, selection=selection, num_classes=num_classes)])
    ...
    def _early_predictor(self, x, empty_indices, idx):
        exit, features, selection = self.exit_module[idx](x)
        with torch.no_grad():
        ...
        return x, exit, features, selection, fill_indices, empty_indices
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x, exit1, features1, select1, fill_indices, empty_indices = self._early_predictor(x, empty_indices, 0)
        ...
        x = self.layer2(x)
        x, exit2, features2, select2, fill_indices, empty_indices = self._early_predictor(x, empty_indices, 1)
        outputs[fill_indices], mark[fill_indices] = exit2, 1
        if x.size(0) == 0:
            return outputs, mark  
        
        x = self.layer3(x)
        x, exit3, features3, select3, fill_indices, empty_indices = self._early_predictor(x, empty_indices, 2)
        outputs[fill_indices], mark[fill_indices] = exit3, 2
        if x.size(0) == 0:
            return outputs, mark
        
        x = self.layer4(x)
        features4 = self.avgpool(x)
        features4 = features4.view(x.size(0), -1)
        exit4 = self.fc(features4)
        ...
        if not self.training:
            ...
            return [exit1, exit2, exit3, exit4], [features1, features2, features3, features4], [select1, select2, select3]
