...
class TrainHandler():
    ...
    def _print_train_stat(self, epoch, num_epochs, epoch_elapse, train_loss, train_acc, valid_loss, valid_acc, learning_rate, early_pred=False):
        ...
        if early_pred:
            # train_acc and valid_acc are list of valid accuracy for each exit paths
            print(('[{}] Loss - {:.4f}, Acc of each path - '.format('Train', train_loss)) + ('{:2.2f}%, ' * len(train_acc)).format(*[math.floor(ta * 10000) / 100 for ta in train_acc]) + 'Learning Rate - {:0.6f}'.format(learning_rate))
            ...
            # train_acc and valid_acc are just a scalar value
    ...
    def train_model(self, num_epochs=200, valid_freq=1, log_freq=1, print_freq=1, early_stop=False, patience=10, verbose=False):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            if self.early_pred:
                if valid_acc[-1] > best_acc or self.dataset_sizes['valid'] == 0:
    ...
    def __result_log_plot(self, result_dict, name, plot_freq):
    ...
    def test_model(self, topk=(1,), plot_freq=0.1, device=None, pretrained=False):
        ...
        if self.early_pred:
            ...
            print(('[{}] Loss - {:.4f}, Acc of each path - '.format('Test', test_loss)) + ('{:2.2f}%, ' * len(test_acc))[:-2].format(*[math.floor(ta * 10000) / 100 for ta in test_acc]))
        else:
        ...
        if not pretrained:
            ...
            if self.early_pred:
                ...
                for idx, tea in enumerate(test_acc):
                    result_dict = {}
                    name = self.name + '/exit_path%d' % idx
                    ...
                    result_dict['train_acc'] = [(tra[0], tra[1][idx]) for tra in self.result_log['train_acc']]
                    result_dict['valid_acc'] = [(va[0], va[1][idx]) for va in self.result_log['valid_acc']]
                    result_dict['test_acc'] = tea

                    self.__result_log_plot(result_dict, name, plot_freq)
        
            else:
                self.__result_log_plot(self.result_log, self.name, plot_freq)
